...
**Writable paths** are matched by canonical path containment (relative to the project dir, symlinks resolved, `..` collapsed): `"src/"` matches `src/foo/bar.py` and any spelling that resolves inside `src/`, but not the sibling `src_secret/`. Exact filenames also work: `"README.md"` matches `README.md` but not `src/README.md` or `README.md.bak`. Absolute or `~`-prefixed glob entries (e.g. `"~/.claude/projects/*/memory/"`) are expanded at check time and matched as patterns. Unknown mode names resolve to `[]` (read-only — the guard fails closed).
