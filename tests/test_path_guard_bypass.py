...
def _project(tmp_path, mode="implement"):
    ...
    (proj / "src").mkdir()
    (proj / "tests").mkdir()
    # The shipped implement default now includes tests/ (test-alongside-code
    # TDD). These bypass tests need a non-writable sibling to escape into,
    # so pin the restricted writable set via a project override.
