...
- **`free` mode is unguarded by design** (`writable = ["*"]`) and is the default. Set a restricted `default_mode` in `.kibitzer/config.toml` if you want the guard active from the first call.
